...
class CLAPScore:
    ...
    def compute_score(
        ...
        assert len(audio_paths) == len(
            texts
        ), "Number of audio files and texts must match"
        ...
        with torch.no_grad():
            for i in tqdm(
                ...
                batch_similarities = (
                    (audio_embeds * text_embeds).sum(dim=1).cpu().numpy()
                )
...
class FADScore:
    ...
    def extract_features(
        ...
        with torch.no_grad():
            for i in tqdm(
                range(0, len(audio_paths), batch_size),
                desc="Extracting features for FAD",
            ):
    ...
    def calculate_score(
        ...
        """Compute FAD score."""
        log.info(
            f"Extracting features from {len(generated_audio_paths)} generated samples..."
        )
        ...
        log.info(
            f"Extracting features from {len(reference_audio_paths)} reference samples..."
        )
        ...
        global_score = self._calculate_frechet_distance(
        ...
        return {"score": global_score, "per_sample_scores": per_sample_scores}
...
class TTAEvaluator:
    ...
    def evaluate(
        ...
        audio_paths = [
            generated_audio_dir / fname for fname in df[filename_column]
        ]
        texts = df[text_column].tolist()

        # Filter out missing audio files
        valid_indices = [
            i for i, path in enumerate(audio_paths) if path.exists()
        ]
        ...
        if compute_fad and reference_audio_dir is not None:
            ...
            if reference_paths:
                ...
                log.warning(
                    f"No reference audio files found in {reference_audio_dir}"
                )
        elif compute_fad:
            log.warning(
                "FAD computation requested but reference_audio_dir not provided."
            )
