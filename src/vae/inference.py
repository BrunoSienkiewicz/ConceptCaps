...
def run_latent_inference(
    ...
    log.info(
        f"Sampling {cfg.inference.num_samples} latent vectors from standard normal..."
    )

    z = torch.randn(
        cfg.inference.num_samples, model.model_cfg.latent_dim, device=device
    )

    with torch.no_grad():
        log.info(
            f"Decoding latent vectors with temperature={cfg.inference.temperature}..."
        )
        ...
        entropy_per_sample = (
            -(
                recon * torch.log(recon + 1e-8)
                + (1 - recon) * torch.log(1 - recon + 1e-8)
            )
            .sum(dim=1)
            .mean()
            .item()
