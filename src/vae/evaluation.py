...
class VAEMetrics:
    ...
    @staticmethod
    def hamming_loss(
        recon: torch.Tensor, target: torch.Tensor, threshold: float = 0.5
    ) -> float:
        """
    ...
    @staticmethod
    def active_units(
        latent: torch.Tensor, threshold: float = 0.0
    ) -> Dict[str, float]:
    ...
    def cluster_separability(
        latent: torch.Tensor, labels: torch.Tensor, metric: str = "silhouette"
    ) -> float:
        ...
        if (labels == 0).all(dim=1 if len(labels.shape) > 1 else 0).any():
            valid_idx = ~(labels == 0).all(
                dim=1 if len(labels.shape) > 1 else 0
            )
    ...
    def _silhouette_score(X: np.ndarray, labels: np.ndarray) -> float:
        ...
        for i in range(len(X)):
            ...
            if len(same_cluster_samples) > 1:
                a = np.mean(
                    np.linalg.norm(sample - same_cluster_samples, axis=1)
                )
            else:
                a = 0.0
            ...
            for other_label in unique_labels:
                if other_label != label:
                    ...
                    if len(other_cluster_samples) > 0:
                        mean_dist = np.mean(
                            np.linalg.norm(
                                sample - other_cluster_samples, axis=1
                            )
                        )
                        b = min(b, mean_dist)
    ...
    def compute_all_metrics(
        ...
        metrics["hamming_loss"] = VAEMetrics.hamming_loss(
            recon, target, threshold
        )
        metrics["jaccard_index"] = VAEMetrics.jaccard_index(
            recon, target, threshold
        )
        ...
        metrics.update(
            {f"active_units_{k}": v for k, v in active_units_metrics.items()}
        )

        # Cluster separability (using argmax of target as pseudo-labels)
        target_labels = (
            target.argmax(dim=1) if len(target.shape) > 1 else target
        )
        metrics["silhouette_score"] = VAEMetrics.cluster_separability(
            latent, target_labels, "silhouette"
        )

        return metrics

    @staticmethod
    def tag_combination_diversity(
        predictions: torch.Tensor, threshold: float = 0.5
    ) -> Dict[str, float]:
        """Compute diversity metrics to detect model collapse (generating same
        tag combinations).
    ...
    def cooccurrence_comparison(
        ...
        # KL divergence
        kl_div = np.sum(
            orig_flat * np.log((orig_flat + 1e-10) / (pred_flat + 1e-10))
        )
        metrics["kl_divergence"] = float(kl_div)

        # Hellinger distance
        hellinger = np.sqrt(
            0.5 * np.sum((np.sqrt(pred_flat) - np.sqrt(orig_flat)) ** 2)
        )
        ...
        if (
            len(pred_reshaped) > 1
            and np.std(pred_reshaped) > 0
            and np.std(orig_reshaped) > 0
        ):
    ...
    def compute_diversity_metrics(
        ...
        target_cooccurrence = VAEMetrics.tag_cooccurrence_matrix(
        ...
        metrics.update(
            {f"cooccurrence_{k}": v for k, v in cooccurrence_metrics.items()}
        )
...
class MetricsSaver:
    ...
    def save_summary(
        self, config: Dict[str, Any], filename: str = "summary.json"
    ) -> Path:
